...
- Ветка main = продакшн. Любой коммит в main автоматически собирается (`npm run build`) и публикуется на partygame-nhatrang.com примерно за 1–2 минуты
- Zip-архивы и ручная загрузка через Cloudflare Dashboard больше не нужны

Как Claude должен вносить правки:
1. Предпочтительный способ — fine-grained GitHub-токен с доступом Contents: Read and write только к этому репозиторию. Токен пользователь даёт в чате, в репозиторий его не записывать. Claude клонирует репозиторий в песочнице, правит, проверяет `npm run build` и пушит в main.
